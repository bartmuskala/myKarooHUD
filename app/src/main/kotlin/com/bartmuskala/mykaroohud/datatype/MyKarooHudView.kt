...
import com.bartmuskala.mykaroohud.datatype.shared.ColorConfig
...
import com.bartmuskala.mykaroohud.datatype.shared.fontSizeForCell
import com.bartmuskala.mykaroohud.datatype.shared.headerHeightPx
import com.bartmuskala.mykaroohud.datatype.shared.renderValueBitmap
import com.bartmuskala.mykaroohud.datatype.shared.toColorConfig
...
// distinguish MyKarooHud vs native cells on stacked 1-col mixed pages.
private const val LAYOUT_PROBE_MODE = false
private const val PROBE_MARKER_COLOR = 0xFFFF00FF.toInt()

fun mykaroohudFieldRemoteViews(
    field: FieldState,
...
private fun layoutRes(
    ...
    ViewConfig.Alignment.RIGHT ->
        if (translationDp == -3) R.layout.mykaroohud_field_neg3
        else R.layout.mykaroohud_field
    ViewConfig.Alignment.LEFT ->
        if (translationDp == -3) R.layout.mykaroohud_field_left_neg3
        else R.layout.mykaroohud_field_left
    ViewConfig.Alignment.CENTER ->
        if (translationDp == -3) R.layout.mykaroohud_field_center_neg3
        else R.layout.mykaroohud_field_center
}
